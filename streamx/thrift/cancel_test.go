...
import (
	...
	"github.com/cloudwego/kitex-tests/streamx/kitex_gen/echo/testcancelservice"
)
...
func (s cancelImpl) CancelBidi(ctx context.Context, stream echo.TestCancelService_CancelBidiServer) (err error) {
	defer func() {
	...
	case bidiStreamingIndependentSendRecv:
		...
		go func() {
			...
			for {
				...
				if rErr == nil {
					test.Assert(t, resp.Message == bidiStreamingIndependentSendRecv, resp)
					continue
				}
		...
		go func() {
			defer wg.Done()
			for {
				sErr := stream.Send(ctx, &echo.EchoClientResponse{Message: bidiStreamingIndependentSendRecv})
				if sErr == nil {
...
func (s cancelImpl) CancelClient(ctx context.Context, stream echo.TestCancelService_CancelClientServer) (err error) {
	defer func() {
		cancelFinishChan <- struct{}{}
	}()
	t := <-s.tChan
...
func (s cancelImpl) CancelServer(ctx context.Context, req *echo.EchoClientRequest, stream echo.TestCancelService_CancelServerServer) (err error) {
	defer func() {
		cancelFinishChan <- struct{}{}
	}()
	t := <-s.tChan
	...
	case serverStreamingRemoteRespondingSlowly:
		...
		for {
			resp := &echo.EchoClientResponse{Message: serverStreamingRemoteRespondingSlowly}
	...
	case serverStreamingDuringNormalInteraction:
		for {
			resp := &echo.EchoClientResponse{Message: serverStreamingDuringNormalInteraction}
			sErr := stream.Send(ctx, resp)
			if sErr == nil {
				time.Sleep(50 * time.Millisecond)
				continue
	...
	case serverStreamingDeferCancel:
		for i := 0; i < 5; i++ {
			resp := &echo.EchoClientResponse{Message: serverStreamingDeferCancel}
			sErr := stream.Send(ctx, resp)
			test.Assert(t, sErr == nil, sErr)
		}
		specialResp := &echo.EchoClientResponse{Message: bizSpecialMessage}
...
func TestCancel(t *testing.T) {
	...
	t.Run(clientStreamingWithoutSendingAnyReq, func(t *testing.T) {
		...
		err = cliSt.Send(cliSt.Context(), &echo.EchoClientRequest{Message: clientStreamingWithoutSendingAnyReq})
	...
	t.Run(clientStreamingDuringNormalInteraction, func(t *testing.T) {
		...
		for {
			err = cliSt.Send(cliSt.Context(), &echo.EchoClientRequest{Message: clientStreamingDuringNormalInteraction})
			if err == nil {
				time.Sleep(50 * time.Millisecond)
				continue
			}
	...
	t.Run(clientStreamingDeferCancel, func(t *testing.T) {
		...
		defer cancel()
		for i := 0; i < 10; i++ {
			err = cliSt.Send(cliSt.Context(), &echo.EchoClientRequest{Message: clientStreamingDeferCancel})
			test.Assert(t, err == nil, err)
		}
	})
	t.Run(serverStreamingRemoteRespondingSlowly, func(t *testing.T) {
		...
		srvSt, err := cli.CancelServer(ctx, &echo.EchoClientRequest{Message: serverStreamingRemoteRespondingSlowly})
		test.Assert(t, err == nil, err)
		time.Sleep(50 * time.Millisecond)
		cancel()
	...
	t.Run(serverStreamingDuringNormalInteraction, func(t *testing.T) {
		...
		srvSt, err := cli.CancelServer(ctx, &echo.EchoClientRequest{Message: serverStreamingDuringNormalInteraction})
		test.Assert(t, err == nil, err)
		go func() {
			time.Sleep(50 * time.Millisecond)
			cancel()
		...
		for {
			...
			if err == nil {
				test.Assert(t, resp.Message == serverStreamingDuringNormalInteraction, resp)
				continue
			}
			verifyClientSideCancelCase(t, srvSt.Context(), err)
			break
	...
	t.Run(serverStreamingDeferCancel, func(t *testing.T) {
		...
		srvSt, err := cli.CancelServer(ctx, &echo.EchoClientRequest{Message: serverStreamingDeferCancel})
		test.Assert(t, err == nil, err)
		defer cancel()
		for {
			resp, err := srvSt.Recv(srvSt.Context())
			...
			if resp.Message == bizSpecialMessage {
				break
			}
			test.Assert(t, resp.Message == serverStreamingDeferCancel)
		}
	})
	t.Run(bidiStreamingIndependentSendRecv, func(t *testing.T) {
		...
		go func() {
			defer wg.Done()
			for {
				sErr := bidiSt.Send(bidiSt.Context(), &echo.EchoClientRequest{Message: bidiStreamingIndependentSendRecv})
				if sErr == nil {
					time.Sleep(50 * time.Millisecond)
					continue
				}
