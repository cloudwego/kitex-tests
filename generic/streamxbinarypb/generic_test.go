...
func TestClientStreaming(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		err = streamCli.Send(streamCli.Context(), buf)
		test.Assert(t, err == nil)
	}
	buf, err := streamCli.CloseAndRecv(streamCli.Context())
	test.Assert(t, err == nil)
	_, ok := buf.([]byte)
	test.Assert(t, ok)
	resp := &pbapi.MockResp{}
...
func TestServerStreaming(t *testing.T) {
	...
	buf := make([]byte, 0)
	buf, _ = req.Marshal(buf)
	streamCli, err := cli.ServerStreaming(ctx, "ServerStreamingTest", buf)
	test.Assert(t, err == nil, err)
	for {
		buf, err := streamCli.Recv(streamCli.Context())
		if err != nil {
