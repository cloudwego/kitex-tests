...
func TestClientStreaming(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		req := map[string]interface{}{"Message": "Hello from Client"}
	...
	test.Assert(t, mp["Message"] == "all message: Hello from Client, Hello from Client, Hello from Client")
}
...
func TestServerStreaming(t *testing.T) {
	...
	for {
		...
		if err != nil {
			...
			test.Assert(t, mp["Message"] == "Hello from Client -> response")
		}
	}
}
...
func TestBidirectionalStreaming(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 3; i++ {
			req := map[string]interface{}{"Message": fmt.Sprintf("Hello from Client %d", i)}
			err = streamCli.Send(streamCli.Context(), req)
			test.Assert(t, err == nil)
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				test.Assert(t, mp["Message"] == "Hello from Server")
			}
		}
	}()
	wg.Wait()
...
func TestUnary(t *testing.T) {
	...
	test.Assert(t, mp["Message"] == "hello Hello from Client")
}
